        ...
        match self.insert_import_comment_with_id(issue_id, comment, &created_at) {
            ...
            Err(BeadsError::Database(error)) if is_import_comment_id_collision(&error) => {
                match self.import_comment_id_owner(comment.id)? {
                    ...
                    // range of a later comment's JSONL id, so restricting the
                    // fallback to cross-issue owners re-inserts with the taken id
                    // and violates the comments PK (same-issue self-collision).
                    Some(_) => {
...
mod tests {
    ...
    */
    ...
    fn test_sync_comments_for_import_handles_same_issue_self_collision_after_auto_realloc() {
        // A corrupt JSONL (e.g. emitted by a bd→br migration) can have the
        // same comment.id on two different issues. The collision handler
        // routes the *second* claimant through AUTOINCREMENT, but
        // AUTOINCREMENT may pick an id that is *also* a JSONL-provided id
        // later in the same issue's comments array. When that later comment
        // is processed, an owner-must-differ guard sees the colliding row
        // owned by the same issue and falls back to the WITH-id INSERT path —
        // which then violates the comments PK because the row was just
        // AUTO-allocated to that id.
        //
        // Repro shape:
        // 1. Pre-existing row owns id=1 (issue X with comment id=1).
        ...
        //      → same-issue owner → must still route through AUTOINCREMENT.
        let mut storage = SqliteStorage::open_memory().unwrap();
    ...
    #[test]
    fn test_sync_comments_for_import_handles_cross_issue_id_collision_during_bulk_import() {
        // Companion coverage: the same corrupt-JSONL shape where two different
        // issues claim one comment.id, imported sequentially with no
        // pre-existing rows anchoring the collision check. The second insert
        // must NOT violate the comments PK.
        let mut storage = SqliteStorage::open_memory().unwrap();
        let t1 = Utc.with_ymd_and_hms(2026, 4, 1, 0, 0, 0).unwrap();

        let issue_a = make_issue("br-cc-a", "Issue A", Status::Open, 2, None, t1, None);
        let issue_b = make_issue("br-cc-b", "Issue B", Status::Open, 2, None, t1, None);
        storage.create_issue(&issue_a, "tester").unwrap();
        storage.create_issue(&issue_b, "tester").unwrap();

        let colliding_id = 330_i64;
        let comment_a = crate::model::Comment {
            id: colliding_id,
            issue_id: "br-cc-a".to_string(),
            author: "alice".to_string(),
            body: "comment from issue A".to_string(),
            created_at: t1,
        };
        let comment_b = crate::model::Comment {
            id: colliding_id,
            issue_id: "br-cc-b".to_string(),
            author: "bob".to_string(),
            body: "different comment, same JSONL id".to_string(),
            created_at: t1 + chrono::Duration::minutes(1),
        };
        ...
        assert_eq!(comments_a.len(), 1);
        assert_eq!(comments_a[0].body, "comment from issue A");

        let comments_b = storage.get_comments("br-cc-b").unwrap();
        assert_eq!(comments_b.len(), 1);
        assert_eq!(comments_b[0].body, "different comment, same JSONL id");
        assert_ne!(
