...
\textbf{Root Cause}: fsqlite (frankensqlite) has a \colorbox{codebg}{\texttt{PageBufPool}} with a hard limit of 1024 page buffers. This pool is \textbf{shared} between the ARC page cache and transaction write-sets (\colorbox{codebg}{\texttt{pager.rs:354: let pool = inner.cache.pool().clone()}}). As the B-tree grows during bulk inserts, dirty pages accumulate in the write-set. The \colorbox{codebg}{\texttt{write\_page}} codepath (\colorbox{codebg}{\texttt{pager.rs:983}}) does \colorbox{codebg}{\texttt{self.pool.acquire()?}} with NO fallback on OOM — unlike \colorbox{codebg}{\texttt{fetch\_page}} which can fall back to reading directly from disk. Once the combined cache + write-set exceeds 1024 slots, every subsequent write fails with \colorbox{codebg}{\texttt{FrankenError::OutOfMemory}}.
