...
\fancyhead[L]{\footnotesize\color{headergray}\textit{Codebase Review: beads\_rust}}
...
% -- Title --
\title{\vspace{-1.8em}\LARGE\bfseries\color{titlebg} Codebase Review: beads\_rust\vspace{-0.3em}}
...
\textbf{Reviewers:} CaseMirror Research \& Claude Opus 4.6
...
\subsubsection{BUG-2: add\_dependency cycle check is outside the transaction (TOCTOU)}
...
\subsubsection{BUG-3: set\_config/set\_metadata non-atomic DELETE+INSERT}
...
\subsubsection{BUG-4: rebuild\_blocked\_cache uses plain BEGIN instead of BEGIN IMMEDIATE}
...
\subsubsection{PERF-1: O(n\^{}2) visited-set check in collect\_descendant\_ids}
